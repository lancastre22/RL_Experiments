...
def reinforce_train(policy, value_function, env, num_epochs, episode_steps, alpha, gamma, device, num_states,
    ...
    for t in range(num_epochs):
        ...
        for _ in range(episode_steps):
            ...
            state_tensor = one_hot_encode(state, num_states).to(device)
        ...
        states_tensor = torch.stack([one_hot_encode(s, num_states) for s in states]).to(device)
        ...
        # Save model if it's the best (using >= to keep the latest one with equal performance)
        if epoch_reward >= best_reward:
            best_reward = epoch_reward
            best_policy_state = policy.state_dict()
            print(f'Epoch {t+1}, Reward: {epoch_reward}, Epsilon: {epsilon:.4f} (New Best)')
...
def test_policy(policy, env, num_episodes, device, num_states, render_delay=2):
...
num_states = env.observation_space.n  # 16 for 4x4 FrozenLake
state_dim = num_states  # Using one-hot encoding
action_dim = env.action_space.n  # 4 for FrozenLake
...
test_rewards = test_policy(policy, test_env, num_episodes=5, device=device, num_states=num_states)
