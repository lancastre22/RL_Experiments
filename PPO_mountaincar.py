...
# Policy Network for continuous actions - adjusted for MountainCarContinuous
class PolicyNet(nn.Module):
    def __init__(self, state_dim, action_dim):
        super(PolicyNet, self).__init__()
        self.fc1 = nn.Linear(state_dim, 64)  # Reduced network size
...
# Value Network - adjusted for MountainCarContinuous
...
# PPO main training function - adjusted for MountainCarContinuous
def ppo_train(policy, value_function, env, num_epochs=200, steps_per_epoch=2000,
              gamma=0.99, lam=0.95, clip_ratio=0.2, pi_lr=3e-4, vf_lr=1e-3,
              train_pi_iters=40, train_v_iters=40, target_kl=0.01, max_ep_len=999,
              batch_size=64, device="cpu"):
    """
    PPO-Clip algorithm implementation for MountainCarContinuous
    """
    # Set up optimizers
    ...
    episode_rewards = []
    episode_lengths = []
    solving_score = 90.0  # Environment defines solving as average of 90.0 over 100 consecutive trials
    ...
    episode_reward = 0
    episode_length = 0
    max_position = -1.2  # Track the furthest position reached
    ...
    for t in range(num_epochs * steps_per_epoch):
        ...
        # Track max position for debugging
        ...
        if done or timeout or epoch_ended:
            ...
            if done or timeout:
                ...
                if episode_reward >= solving_score:
                    consecutive_solves += 1
                    if consecutive_solves >= 100 and not solved:
                        print(f"Environment solved in {len(episode_rewards)} episodes!")
                        solved = True
                else:
                    consecutive_solves = 0
        ...
        if epoch_ended:
            ...
            for i in range(train_pi_iters):
                ...
                # Add entropy bonus for exploration (critical for sparse reward environments)
                ...
                # Check KL divergence to prevent too large policy updates
                if (i+1) % 10 == 0:
            ...
            if len(episode_rewards) > 0:
                ...
                print(f"Epoch {epoch}/{num_epochs} | Mean Reward: {mean_reward:.2f} | Max Position: {max_position:.4f}")
                ...
                if mean_reward > best_reward:
                    ...
                    if mean_reward >= solving_score and len(episode_rewards) >= 100:
                        print(f"Solved! Mean reward over last 100 episodes: {mean_reward:.2f}")
                        break
...
def test_policy(policy, env, num_episodes=5, device="cpu"):
    ...
    for episode in range(num_episodes):
        ...
        print(f"Test Episode {episode+1}: Reward = {episode_reward:.2f}, Max Position = {max_position:.4f}")
...
def main():
    ...
    state_dim = env.observation_space.shape[0]  # 2 for MountainCarContinuous
    action_dim = env.action_space.shape[0]      # 1 for MountainCarContinuous
    ...
    best_policy_state, best_reward = ppo_train(
        ...
        steps_per_epoch=2000,      # Steps per epoch
        ...
        max_ep_len=1000,            # Maximum episode length (matches environment)
    ...
    model_save_path = "mountain_car_continuous_ppo_model.pth"
